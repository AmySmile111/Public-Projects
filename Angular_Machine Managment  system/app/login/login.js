...
var LoginComponent = (function () {
    ...
    LoginComponent.prototype.onSubmit = function (name, password) {
        var _this = this;
        this.authService.checkAdmin(name);
        this.authService.login(name, password).subscribe(function () {
            if (_this.authService.isLoggedIn) {
                if (_this.authService.addrouter && _this.authService.admin) {
                    _this.router.navigate(['/addUpdate']);
                }
                if (_this.authService.addrouter && !_this.authService.admin) {
                    _this.router.navigate(['/filter']);
                    alert("You are not administrtaor.You can not have access to add and upadate machine page");
                }
                if (_this.authService.updateM && _this.authService.admin) {
                ...
                if (_this.authService.firouter) {
                    _this.router.navigate(['/filter']);
                }
                if (!_this.authService.addrouter && !_this.authService.updateM && !_this.authService.firouter) {
                    _this.router.navigate(['/filter']);
                }
            }
            else {
